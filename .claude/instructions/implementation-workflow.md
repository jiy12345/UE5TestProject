...
7. **구현 커밋 (코드 변경만)**:
   - 단계의 실제 구현 (소스 코드, Build.cs 등)
   - 코드 안의 함정 한 줄 주석은 이 커밋에 포함 (코드와 분리 불가)
   - 커밋 메시지 type/scope = 변경 성격 (`feat`, `build` 등)
8. **정리 커밋 (인사이트 문서만)**:
   - `Docs/Practical/...` 신규/갱신
   - 기존 `Docs/AI/...` 등 개념 문서 보완
   - 커밋 메시지 type = `docs`
   - 같은 단계 N의 두 번째 커밋임을 표시 (예: `[#19 단계 N · 정리]`)
9. **푸시** — 두 커밋(구현 + 정리)을 함께 푸시
10. **PR 본문 체크리스트 갱신** (`gh pr edit`) — 1단계라면 Draft PR 신규 생성
11. **사용자에게 PR/커밋 링크 알리고 리뷰 대기** — Claude는 다음 단계 시작 X
12. **사용자 리뷰 채널**:
    - GitHub PR 인라인 코멘트 (`gh api repos/<owner>/<repo>/pulls/<num>/comments`로 확인)
    - 채팅 직접 피드백
    - 둘 다 OK — Claude는 양쪽 다 확인하고 응답
13. **사용자가 "다음 단계 진행" 등 명시적 OK 주면** 다음 단계 시작
14. **수정 요청 시**: 같은 브랜치에 **추가 커밋** (rebase/amend 금지 — 학습 흐름 보존)
15. **납득되지 않으면 머무름** — 사용자가 "이해 안 됨"이라고 하면 절대 다음 단계로 넘어가지 않음

### 단계별 커밋 메시지 형식 (구현 / 정리 분리)
...
```
<type>(<scope>): [#<이슈번호> 단계 N · 구현] <단계 제목>

<설계/구현 요약 1~2문단>

(설계 결정) <한 줄, 본 커밋 본문에 자세히>
...
```
docs(<scope>): [#<이슈번호> 단계 N · 정리] <단계 제목>

<단계 N에서 도출된 인사이트 한 줄 요약>
...
이유:
- **코드 리뷰 깔끔** — 구현 커밋만 보면 코드 변경 한눈에 파악
- **문서 변경 추적 분리** — 정리 커밋만 보면 학습 정리 추이 파악
- **추적성은 단계 번호로 유지** — `[#19 단계 2 · 구현]` / `[#19 단계 2 · 정리]`로 짝짓기 가능
...
- **구현과 인사이트 정리를 한 커밋에 묶기** — 코드 리뷰 가독성 위해 분리 필수 (단계 번호로 짝짓기)
