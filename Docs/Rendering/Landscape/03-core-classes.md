...
#### 그래서 "프록시가 마스터를 참조해야 하는 경우"는 언제인가

WP 맵에서 스트리밍 프록시가 마스터 `ALandscape`를 실제로 참조하는 경우는 **제한적**입니다:

| 상황 | 마스터 참조 이유 | 빈도 |
|------|-------------|------|
| **공유 속성 fixup** | 프록시 로드 시 마스터 값 흡수 (`FixupOverriddenSharedProperties`) | 로드 1회 + 마스터 변경 시 |
| **편집 세션 중** | 사용자가 브러시·페인트할 때 마스터의 `EditingLayer`, `LandscapeEditLayers[]` 참조 | 에디터 편집 활성 시 |
| **Edit Layer 머지 트리거** | BatchedMerge가 마스터에서 시작되어 영향 컴포넌트 결정 시 프록시 컴포넌트들 조회 | 편집 후 |
| **GUID 검증 (`IsValidLandscapeActor`)** | 프록시 스폰 시 마스터의 `LandscapeGuid`와 일치 확인 | 로드 1회 |
| **공통 그리드 정책 조회** | `GetGridSize`, `bAreNewLandscapeActorsSpatiallyLoaded` 등 | 에디터 작업 시 |
| **LOD 그룹 키 조회** | 같은 그룹 프록시들 LOD 조율 시 | 매 프레임 (간접) |

즉 **사용자 결론대로**: 마스터 참조의 거의 대부분이 **(a) 공유 속성 흡수/조회** + **(b) 편집 세션 동안의 편집 상태 조회**입니다.

**런타임 게임에서**: 일반적으로 편집이 없으므로 (a)는 첫 로드 1회뿐, (b)는 발생하지 않음 → **프록시가 마스터를 자주 참조하지 않습니다**. 렌더링·물리 쿼리는 프록시 자체의 데이터(컴포넌트, 텍스처)만으로 완결.

**왜 이런 분리 설계인가**: 마스터를 "권위자"로, 프록시를 "데이터 보유자 + 자급자족 렌더 단위"로 만들어, 마스터가 메모리 외 시점에도 (이론상으로는) 프록시는 독립 동작 가능하게 함. 실제로 마스터는 항상 로드 상태이지만, 의존도가 낮을수록 시스템이 견고해집니다.
