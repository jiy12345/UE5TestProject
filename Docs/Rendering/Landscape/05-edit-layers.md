...
##### 에디터 메모리·GPU 측면 비용 상세

"런타임에는 영향 없다"가 핵심이지만, **에디터 작업 중에는** 텍스처 자체가 늘어나는 데서 오는 비용이 분명히 있습니다:

| 비용 | 구체적 영향 |
|------|---------|
| **에디터 GPU 메모리** | LayersData의 각 레이어별 기여 텍스처가 GPU에 업로드 → 메모리 = (레이어 수) × (컴포넌트 수) × (텍스처 크기) |
| **CPU 메모리** | Persistent 레이어의 BulkData가 CPU 측에도 보관됨 |
| **BatchedMerge 텍스처 바인딩** | 머지 패스가 모든 레이어 텍스처를 샘플링 → 셰이더가 바인딩해야 할 텍스처 수 증가, 한계 있음 (예: 16~32개 동시 바인딩) |
| **머지 셰이더 컴파일 시간** | 레이어 수에 따라 셰이더 변종 생성, 첫 머지 시 미세한 컴파일 지연 |
| **디스크 사용량** | `.uasset` 저장 시 모든 LayersData 직렬화 → 프로젝트 디스크 사용량 증가 |
| **DDC 캐시 용량** | 머지 결과 캐시도 늘어남 |

**병목 가능성 큰 곳 = BatchedMerge 시간**:
- 한 머지 패스가 N개 레이어 → 각 레이어 RT에 N번 렌더 → 시간이 N에 선형 비례
- 컴포넌트가 100개, 레이어가 20개 → 한 번의 전체 머지에 2000번 렌더 = 수 초 단위 가능
- 사용자가 브러시 한 번 클릭할 때마다 부분 머지 발생하므로 **편집 응답성이 떨어질 수 있음**

**현실적 영향**:
- 에디터에서 레이어 5~10개 수준은 무리 없음
- 20~50개 넘어가면 머지 시간이 눈에 띄게 길어져 편집 워크플로우 답답해짐
- 그래서 권장 사용 패턴은 **"레이어를 무한정 쌓지 말고, 일부는 Collapse(병합) 처리"** — `ALandscape::CollapseLayer` API로 여러 레이어를 하나로 합쳐 데이터·머지 비용 회수

따라서 정확한 문장은: **"게임 빌드 런타임 비용은 영향 없음. 에디터 편집 시간·메모리·디스크 비용은 레이어 수에 비례 증가."**
...
#### "위에 쌓이는 형식이 맞나" — 포토샵 레이어와의 일대일 대응

**네, 정확히 포토샵 레이어 모델입니다.** 시각적으로도 동작 측면에서도 같은 패턴:

| 포토샵 | Landscape Edit Layers |
|---------|----------------|
| 레이어 패널 (위에서 아래) | `LandscapeEditLayers[]` 배열 (순서 바꾸기 가능) |
| 각 레이어 = 색상·알파 정보 | 각 레이어 = Δ높이 + Δ가중치 + (선택) 가시성 |
| 가시성 토글 (눈 아이콘) | `ULandscapeEditLayerBase::bVisible` 플래그 |
| 알파 슬라이더 | 레이어 알파 (`HeightmapAlpha`/`WeightmapAlpha`) |
| 블렌드 모드 (Multiply, Add, Overlay 등) | 블렌딩 방식 (Additive, Alpha 등) |
| 위 레이어가 아래를 가림 | 상위 Edit Layer Δ가 누적 적용되어 결과를 변경 |
| 레이어 병합 (Merge Down) | `ALandscape::CollapseLayer` API |

**시각적으로 "위에 쌓이는" 메커니즘**:
- 머지 시 **아래 레이어부터 위로** 순차적으로 RT에 그려짐
- 각 레이어가 자기 Δ를 RT에 누적 (이미 그려진 결과 위에)
- 최종 RT의 픽셀 값 = 모든 레이어의 Δ가 누적된 결과

**다른 점**:
- 포토샵은 픽셀 단위 색상이지만, Landscape는 Heightmap(높이)·Weightmap(레이어 가중치)·Visibility 세 종류 데이터를 동시에 다룸
- 포토샵의 "한 레이어 = 한 종류 정보"가 아니라 "한 Edit Layer = (Height + Weight + Visibility) 묶음"
- 따라서 한 Edit Layer가 동시에 산을 만들고(Height Δ) + 그 위에 풀을 칠하는(Weight Δ) 식의 복합 동작 가능

요약: **편집 모델 자체는 포토샵 레이어와 거의 동일**하고, 차이는 "다루는 데이터의 종류가 색상 1개가 아니라 Heightmap/Weightmap/Visibility 3종류 묶음"이라는 점입니다.
...
한 번에 대량으로 명령을 묶어 보내는 것이 수시로 여러 번 보내는 것보다 훨씬 효율적입니다. 3가지 이유:
...
#### 머지 자체는 GPU 전담 작업 — 단계별 위치 정리

**컴포넌트 머지의 핵심 합성 계산(Heightmap/Weightmap)**은 **온전히 GPU에서** 이루어집니다. 단계별로 정확히 구분하면:
