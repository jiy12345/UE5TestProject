...
설정값별 컴포넌트 정점 격자 시각 비교:

```
[A] SubsectionSizeQuads=7, NumSubsections=1       [B] SubsectionSizeQuads=15, NumSubsections=1
    → ComponentSizeQuads=7 (8×8 정점)                 → ComponentSizeQuads=15 (16×16 정점)
    
    +---+---+---+---+---+---+---+                    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    |   |   |   |   |   |   |   |                    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    +---+---+---+---+---+---+---+                    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    |   |   |   |   |   |   |   |                    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    +---+---+---+---+---+---+---+                    (16×16 정점, 가까이 보면 더 촘촘)
    |   |   |   |   |   |   |   |                    
    +---+---+---+---+---+---+---+                    소형 지형/모바일에서 사용
    (낮은 해상도)


[C] SubsectionSizeQuads=31, NumSubsections=2     [D] SubsectionSizeQuads=63, NumSubsections=2
    → ComponentSizeQuads=63 (64×64 정점)              → ComponentSizeQuads=127 (128×128 정점)
                                                     
    ┌─────────────┬─────────────┐                    ┌─────────────────────┬─────────────────────┐
    │  Sub(0,0)   │  Sub(1,0)   │                    │      Sub(0,0)       │      Sub(1,0)       │
    │  32×32      │  32×32      │                    │      64×64          │      64×64          │
    │             │             │                    │                     │                     │
    │             │             │                    │                     │                     │
    ├─────────────┼─────────────┤                    ├─────────────────────┼─────────────────────┤
    │  Sub(0,1)   │  Sub(1,1)   │                    │      Sub(0,1)       │      Sub(1,1)       │
    │  32×32      │  32×32      │                    │      64×64          │      64×64          │
    │             │             │                    │                     │                     │
    └─────────────┴─────────────┘                    └─────────────────────┴─────────────────────┘
    (소~중형 지형/프로토타입)                           (대규모 야외 맵 — 가장 흔한 기본값)
```

**설정값별 정점 수 요약**:

| 구성 | ComponentSizeQuads | 정점 수 (=(N+1)²) | 메모리 (정점 버퍼만) | 흔한 사용처 |
|------|----|----|----|------|
| A | 7 | 64 | ~512 B | 초소형/디버그 |
| B | 15 | 256 | ~2 KB | 모바일/저해상 |
| C | 63 | 4,096 | ~32 KB | 중간 규모 |
| D | 127 | 16,384 | ~128 KB | 일반 야외 (기본값) |

> 격자가 촘촘할수록 한 컴포넌트가 표현할 수 있는 지형 디테일이 풍부해지지만, 컴포넌트 수 × 정점 수가 폭증하면 GPU 처리량과 BatchedMerge 비용이 함께 늘어남. 일반적으로 **D (127×127, 2×2 subsection)**가 표준 기본값.
